...
interface RueckfahrtBoxProps {
  journey: JourneyDetails;
}

export default function RueckfahrtBox({ journey }: RueckfahrtBoxProps) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 mt-10 mb-8 w-full mx-auto relative overflow-hidden">
      <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
      <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
        <Home size={20} className="text-blue-600" /> Entspannte Rückreise — Sonntagabend
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {journey.transferBuffer && (
          <div className="flex items-start gap-2 bg-amber-50/50 p-3 rounded-lg border border-amber-100">
            ...
            <div>
              <span className="font-semibold block text-amber-900 text-sm">Sicherer Umstieg</span>
              <p className="text-amber-800 text-sm">{journey.transferBuffer}</p>
            </div>
          </div>
        )}
        
        {journey.arrivalHomeAdvice && (
          <div className="flex items-start gap-2 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
            <AlertCircle size={16} className="text-blue-600 mt-0.5 flex-shrink-0" />
            <div>
              <span className="font-semibold block text-blue-900 text-sm">Montagsvorbereitung</span>
              <p className="text-blue-800 text-sm">{journey.arrivalHomeAdvice}</p>
            </div>
          </div>
        )}
      </div>
